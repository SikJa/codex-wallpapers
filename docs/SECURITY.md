...
El indicador del perfil lee el porcentaje de la consulta de uso que Codex ya mantiene y solicita su actualización cada 30 segundos cuando la ventana está visible y el dato está desactualizado. No usa una autenticación separada ni guarda o exporta ese dato. No lee conversaciones, cookies ni credenciales. Los archivos importados se tratan como medios; no se ejecutan páginas web, scripts ni escenas. El mod necesita acceso al DOM para decorar la interfaz. Un bug puede afectar legibilidad, rendimiento o controles; no se promete ausencia de fallos.
