...
try{
 ...
 await page.evaluate(id=>window.__CODEX_WALLPAPERS_PUBLIC__.select(id),movie.id);await page.waitForTimeout(300);assert.equal(await page.locator('#cw-media').evaluate(e=>e.videoWidth),640);assert.equal(await page.locator('#cw-media').evaluate(e=>e.muted),true);assert.equal(await page.locator('#cw-media').evaluate(e=>e.paused),false);
 ...
 const second=await context.newPage();await second.goto(url);await applyWindow({evaluate:x=>second.evaluate(x)},await readLibrary(data),data,code);assert.equal((await second.evaluate(()=>window.__CODEX_WALLPAPERS_PUBLIC__.status())).selected,movie.id);await second.evaluate(id=>window.__CODEX_WALLPAPERS_PUBLIC__.select(id),image.id);await page.waitForTimeout(300);assert.equal((await page.evaluate(()=>window.__CODEX_WALLPAPERS_PUBLIC__.status())).selected,image.id);results.push('Second window + synchronized saved selection');
 // A corrupt media entry must never replace the active background.
 const failure=await page.evaluate(async()=>{const api=window.__CODEX_WALLPAPERS_PUBLIC__,id='aaaaaaaaaaaaaaaaaaaaaaaa',before=api.status().selected;api.append(id,btoa('broken'));api.register({id,title:'Broken',kind:'image',mime:'image/png',size:6,width:2,height:2},'');try{await api.select(id);return false}catch{return api.status().selected===before&&document.querySelectorAll('#cw-media').length===1;}});assert.equal(failure,true);results.push('Failed media selection preserves previous wallpaper');
